...
export default function BookingsTable({ 
  ...
  // Optimistic UI Hook
  const [optimisticBookings, setOptimisticStatus] = useOptimistic(
    initialBookings,
    (currentBookings, action: { bookingId: string; newStatus: Booking['status'] }) =>
      currentBookings.map((b) =>
        b.id === action.bookingId ? { ...b, status: action.newStatus } : b
      )
  )

  const displayedBookings = limit ? optimisticBookings.slice(0, limit) : optimisticBookings
  ...
  const handleAction = async (
    ...
    startTransition(async () => {
      // 1. Immediately change status in UI
      setOptimisticStatus({ bookingId, newStatus })

      // 2. Perform backend server action
      const result = await actionFn(bookingId)

      if (result.success) {
        toast.success(result.message || `Status updated`)
        // 3. Purge cache on client
        router.refresh()
      } else {
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          {limit ? `Showing latest ${limit} bookings` : "All your bookings in one place"}
        </CardDescription>
      </CardHeader>
      <CardContent>
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {displayedBookings.map((booking, index) => {
                  const isCurrentProcessing = processingId === booking.id && isPending
                        ...
                        <div className="flex gap-2 flex-wrap">
                          {booking.status === 'REQUESTED' && (
                            <>
                              <Button
                                ...
                                onClick={() => handleAction(booking.id, 'ACCEPTED', acceptBooking)}
                              ...
                              <Button
                                ...
                                size="sm"
                                variant="destructive"
                                onClick={() => handleAction(booking.id, 'DECLINED', declineBooking)}
                                disabled={isCurrentProcessing}
                              >
                                {isCurrentProcessing ? (
                                  <Loader2 className="h-4 w-4 animate-spin" />
                          ...
                          {booking.status === 'PAID' && (
                            <Button
                              ...
                              onClick={() => handleAction(booking.id, 'IN_PROGRESS', markInProgress)}
                              disabled={isCurrentProcessing}
                            >
                              {isCurrentProcessing ? (
                                <Loader2 className="h-4 w-4 animate-spin" />
                          ...
                          {booking.status === 'IN_PROGRESS' && (
                            <Button
                              ...
                              onClick={() => handleAction(booking.id, 'COMPLETED', markCompleted)}
                              disabled={isCurrentProcessing}
                            >
                              {isCurrentProcessing ? (
                                <Loader2 className="h-4 w-4 animate-spin" />
